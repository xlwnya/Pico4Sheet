...
* 接点の配置の問題か、現在の設計では中指が立ちやすいです。導電性のあるテープ等により接点を延長すると改善します。
...
* 製造設定
  ...
  * EDA Software: Other (実際のところファイル自体はEasyEDA Proで作成されていますが、その場合少量(5枚)での特別価格($2)が適用されないためOtherにする必要があります(サポートにOtherと指定しても問題ないことは確認済))
  * Stiffener: polyimide
...
* Pico4Sheet_v7.rsdocx, Pico4Sheet_v7.stp
  * 設計データです。作成中のデータについては訳が分からない状態になっているためほぼ完成データのみです。
* stl
  * 3Dプリント用データです。
  * v7でFFF式の3Dプリンタで印刷しやすいように修正されました。ビルドプレートとの接地面積確保のため多少角ばっています。
* フタのねじはM1.7 x 5mmのタッピングねじを使用しています。
* 現状の問題点
...
* フタを付けて固定する(任意)
  * M1.7 x 5mm のタッピングねじを使用することで固定できるはずです。
